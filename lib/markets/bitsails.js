...
var base_url = 'https://api.bitsails.com';

function get_summary(coin, exchange, cb) {
  var req_url = base_url + '/markets/GetMarketInfo/?MarketName=' + exchange + '-' + coin;
...
function get_trades(coin, exchange, cb) {
  var req_url = base_url + '/markets/GetMarketInfo/?MarketName=' + exchange + '-' + coin + '&count=50';
  request({uri: req_url, json: true}, function (error, response, body) {
...
function get_orders(coin, exchange, cb) {
  var req_url = base_url + '/orders/GetOrderBookData/?MarketName='  + exchange + '-' + coin + '&type=both' + '&depth=50';
  request({uri: req_url, json: true}, function (error, response, body) {
    if (body.success == true) {
